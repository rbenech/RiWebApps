...
var message = React.createClass({
    render: function() {
        switch (SM._kind) {
            case 'toppane':
                document.title = SM.label;
                for (let i = 0; i < Object.keys(this.subpane).length - 1; i++) {
                     this.subpane[i].render()
                     }
                return React.createElement('div', {id: SM.identifier})
            case 'subpane':
                switch (SM.class) {
                case 'Button':
                    ...
                    default:
                        return React.createElement('button', {id: this.identifier, className: this.class, style: makeStyleFromFrameRatio(getFrameRatioFor(this.frameRatio))});
                    }
                   case 'TextPane':
...
ReactDOM.render( sysMsgs, document.getElementById('whiteboard'))
