...
/**-----------------------------------------------------------------------------------------
  Turtle panel. 
  This is the whiteboard "turtle panel" widget. It constructs a view based on turtle commands.
  
  Commands are: 3dbox, box, centerText, charBox, clickRegion, displayText, fill, foreColor, help, 
                line, lineWidth, paneSize, place, widget, widgetEnd
  
  SAMPLE: 'widget:7; clickRegion:121@33@171@8; place:121@32; foreColor:ClrPalegray;fill: 171@9; 
           3Dbox:121@33@171@8; foreColor:ClrBlack; lineWidth:1; charBox:10@10; place:146@20; 
           centerText:10; place:121@34; displayText:Low Band Attn; place:120@33' 
  
  3/14/2012 oer
  ------------------------------------------------------------------------------------------
*/
//TODO: group items (svg 'g' element) by widget. Place a help hover on the group
...
ri.turtle = (function () {
  ...
  //------------------------
  var 
    charBoxVal, currentSvgGroup, 
    //currentTwidget, 
    fontSize, 
    foreColor, lineWidth, namedRegions, namespace1999, namespace2000, panelWidgetId,
    paneSize, place, tooltip, top, twidgets;
  ...
  var doSvgEvent_click = function(evt) {
    ...
    //ri.widgets.emitSmEvent(id, 'do event for '+(evt.altKey ? 'ALT ' : '')+(evt.ctrlKey ? 'CTL ' : '')+(evt.metaKey ? 'META ' : '')+(evt.shiftKey ? 'SHIFT ' : '')+ '('+evt.clientX+', '+evt.clientY+')'); //debug
    ri.widgets.emitSmEvent(panelWidgetId, turtleWidgetId, getMouseEventNameFor('Up', evt));
  }
  
  /**This is used to emit sm events for svg items: mouse down*/
  var doSvgEvent_down = function(evt) {
    var turtleWidgetId = evt.target.turtleWidgetId || 'no-ID'; //also currentTarget
    ri.widgets.emitSmEvent(panelWidgetId, turtleWidgetId, getMouseEventNameFor('Down', evt));
  }
 ...
 /**Draws the provided turtle content using svg (scalar vector graphics). 
  ...
  var setDrawContent = function(parentWidgetId, contents) {
    ...
    var svgElem = initializeTurtlePanel(parentWidgetId, 'svgElem'); //clears the svg element, creates and installs it if necessary, initializes variables

    for(ndx in contents) { //go thru each of the received commands
      //console.log('  TURTLE::'+contents[ndx]); //debug

      //Parse the given command and args:
      var keyval=contents[ndx];
      var cmd=null; //given 'cmd:args', separate out into two strings
      var args=null;
      if(keyval) {
        ...
        switch(cmd[0]) {
          ...
          case 'l':
            if(cmd==='line') { //draw a line. [line:name1;name2] or [line:x1@y1;x2@y2]
              var sa = args.split(';');
              var len = sa.length;
              if(len<2) console.log('TurtlePanel. Error in "line" cmd: illegal arg: "'+arg+'"');
              else {
                var c0 = namedRegions[sa[0]];    //try for a named region
                if(!c0) c0 = parseCoords(sa[0]); //failing that, take it as an absolute position
                var c1 = namedRegions[sa[1]];    //try for a named region
                if(!c1) c1 = parseCoords(sa[1]); //failing that, take it as an absolute position
                if(!c0 || !c1) console.log('TurtlePanel. Error in "line" cmd: needs two arguments: "'+arg+'"');
                else { //draw the line
                  var elem = createSvgElement( 
          ...
          case 'p':
            if(cmd==='panesize') { //given a string of the form 0@218@443@0 representing left@top@right@bottom
              var c = parseCoords(args); //'0@218@443@0' to { x:0, y:218, width:442, height:218 }
      ...
      if(!handled) 
        console.log('TurtlePanel: Unrecognized command: '+cmd+':'+args);
    } //for
